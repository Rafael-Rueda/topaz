...
function updateDashboardPackageJson(projectPath, projectName) {
    const dashboardPkgPath = path.join(projectPath, "dashboard", "package.json");
    if (fs.existsSync(dashboardPkgPath)) {
        const packageJson = JSON.parse(fs.readFileSync(dashboardPkgPath, "utf8"));

        packageJson.name = `${projectName}-dashboard`;
        packageJson.version = "0.0.1";

        fs.writeFileSync(dashboardPkgPath, JSON.stringify(packageJson, null, 4) + "\n");
    }
}
...
async function main() {
    ...
    try {
        ...
        try {
            ...
            log("  Skipped npm install (you can run it manually later)", COLORS.dim);
        }
        ...
        try {
            ...
            log("  Skipped dashboard npm install (you can run it manually later)", COLORS.dim);
        }
